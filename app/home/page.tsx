...
export default function CustomerHomePage() {
  ...
  // First-visit onboarding — mobile only, shown once, then never again.
  useEffect(() => {
    try {
      const isMobile = window.matchMedia("(max-width: 767px)").matches;
      const seen = window.localStorage.getItem(ONBOARDING_SEEN_KEY) === "true";
      if (isMobile && !seen) router.replace("/welcome");
    } catch { /* private mode, etc. — just skip onboarding rather than block the app */ }
    ...
    <div className="home-page min-h-screen bg-[#F4F7F4] pb-28 md:pb-0 animate-fade-in">
      ...
      <header className="border-b border-emerald-100 bg-white">
        ...
        {showMenu && <nav className="border-t border-gray-100 px-5 py-3 lg:hidden">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-3 text-sm font-semibold text-gray-600"><Link href="/home">Home</Link><Link href="/shops">Shop</Link><Link href="/offers">Offers</Link><Link href="/messages" className="relative flex items-center gap-1"><MessageCircle size={14} /> Messages{unreadMessages > 0 && <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-[#E84919] px-1 text-[9px] font-bold text-white">{unreadMessages}</span>}</Link><Link href="/assistant" className="flex items-center gap-1 text-[#16A34A]"><Sparkles size={14} /> MamaFresh AI</Link><Link href="/about">About</Link><Link href="/contact">Contact</Link></div>
        </nav>}
      </header>

      <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          <button onClick={() => setShowLocationModal(true)} className="flex items-center gap-2 text-left text-sm font-bold text-[#073729]"><MapPin size={17} className="text-[#84CC16]" /><span><span className="block text-[10px] font-medium uppercase tracking-wider text-gray-400">Deliver to</span>{location}</span><ChevronDown size={15} className="text-gray-400" /></button>
          <Link href="/search" className="hidden items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-2 text-xs text-gray-400 shadow-sm sm:flex lg:w-80"><Search size={15} /><span>Search fresh vegetables, fruits...</span></Link>
          <Link href="/shops" aria-label="Filter products" className="flex h-10 w-10 items-center justify-center rounded-full bg-[#84CC16] text-[#073729] shadow-sm"><SlidersHorizontal size={17} /></Link>
        </div>

        <section className="relative overflow-hidden rounded-[28px] bg-[#073729] px-6 py-10 text-white sm:px-10 lg:min-h-[360px] lg:px-14 lg:py-14">
