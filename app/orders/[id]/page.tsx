...
const DeliveryMap = dynamic(() => import("@/components/DeliveryMap"), { ssr: false, loading: () => <div className="h-64 w-full animate-pulse rounded-xl bg-gray-100" /> });
...
export default function OrderTrackingPage({ params }: Props) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void fetch(`/api/geocode/forward?q=${encodeURIComponent(order.delivery_address)}`)
      .then((res) => res.json() as Promise<{ lat?: number; lng?: number; error?: string }>)
      .then((result) => {
        if (!active) return;
        if (result.lat != null && result.lng != null) setDestCoords({ lat: result.lat, lng: result.lng });
        else setDestError(result.error ?? "Couldn't locate the delivery address on the map.");
      })
