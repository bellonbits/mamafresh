...
async function uploadViaSignedEndpoint(file: File, signEndpoint: string): Promise<string> {
  const signRes = await fetch(signEndpoint, { method: "POST", headers: await getAuthHeaders() });
...
export function uploadProductImage(file: File): Promise<string> {
  return uploadViaSignedEndpoint(file, `${API_BASE_URL}/api/admin/cloudinary-sign`);
}
...
export function uploadAvatarImage(file: File): Promise<string> {
  return uploadViaSignedEndpoint(file, `${API_BASE_URL}/api/account/avatar-sign`);
}
