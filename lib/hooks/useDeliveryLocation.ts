...
export function useDeliveryLocation() {
  ...
  const detectCurrentLocation = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const result = await response.json() as { label?: string; error?: string };
      if (!response.ok || !result.label) throw new Error(result.error ?? "Unable to determine your address.");
